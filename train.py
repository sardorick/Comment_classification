...
from data_processer import load_data, train_test_split, preprocessing, encoder, token_encoder, padding
...
for epoch in range(num_epochs):
    ...
    for i, (sentences, labels) in enumerate(iter(train_loader)):
        ...
        features = df['comment'].to(device)
        target1 = df['toxic'].to(device)
        target2 = df['severe_toxic'].to(device)
        target3 = df['obscene'].to(device)
        target4 = df['threat'].to(device)
        target5 = df['insult'].to(device)
        target6 = df['identity_hate'].to(device)
